...
>
			...
			<head>
				<title>
					<xsl:choose>
						...
						<xsl:otherwise>Indice della sitemap</xsl:otherwise>
					</xsl:choose>
				</title>
	...
	<a href="./index.html"><span>Home</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<span>Indice della sitemap</span>
	...
	<a href="./index.html"><span>Home</span></a>
			...
			<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
					...
					<a href="./sitemap.xml"><span>Indice della sitemap</span></a>
						<svg width="6" height="8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M.727 7.06L3.78 4 .727.94l.94-.94 4 4-4 4-.94-.94z" fill="#141B38"/></svg>
	...
	<div id="content-head">
		...
		<p>Generato da <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank">All in One SEO</a>, si tratta di una sitemap XML, pensata per essere usata dai motori di ricerca come Google o Bing.</p>
		<p>
			Trovi maggiori informazioni sulle sitemap XML su <a href="https://www.sitemaps.org/" target="_blank" rel="noreferrer noopener">sitemaps.org</a>.		</p>
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				<xsl:choose>
					<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">
						Questa sitemap contiene						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
					...
					<xsl:otherwise>
						L'indice della sitemap contiene						<xsl:value-of select="$amountOfURLs"/>
						<xsl:choose>
							<xsl:when test="$amountOfURLs = 1">
								sitemap							</xsl:when>
							<xsl:otherwise>
								sitemap							</xsl:otherwise>
						</xsl:choose>
					</xsl:otherwise>
				</xsl:choose>
				ed è stata generata il Settembre 1, 2023 alle 4:07 pm			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Bassa</xsl:when>
	...
	<a href="./index.html"><span>Home</span></a>
		...
		<h2 class="empty-sitemap__title">
			Oops!			<br />
			Non ci sono articoli qui		</h2>
		<div class="empty-sitemap__buttons">
			<a href="./index.html" class="button">Torna alla homepage</a>
					</div>
